...
for (i in 1:length(train_names)){
  for (n in 1:length(variables)){
    snaivef <- snaive(get(paste0("train",i))[,n], h=1)
    print(paste0("train",i))
    print(colnames(get(paste0("train",i))[,n]))
...
Tempdata <- read.csv("D:\\T1\\Forecasting\\California_temp_max_min.csv")
tstemp <- xts(Tempdata[,2:3],as.Date(Projectdata$ï..date))
